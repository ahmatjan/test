...
/**   
  ...
  */    
    ...
    public static boolean sendHtmlMail(MailSenderInfo mailInfo){    
      ...
      String[] ccStrings = mailInfo.getReceivers();
      
      if (null!=ccStrings) {
    	  Address[] ccAddress = new Address[ccStrings.length];
		for (int i = 0; i < ccStrings.length; i++) {
			ccAddress[i] = new InternetAddress(ccStrings[i]);
		}
		mailMessage.setRecipients(Message.RecipientType.CC, ccAddress);
      }
      
      String[] replyToStrings = mailInfo.getReplyTo();
      if (null!=replyToStrings) {
    	  Address[] replyToAddress = new Address[replyToStrings.length];
    	  for (int i = 0; i < replyToStrings.length; i++) {
    		  replyToAddress[i] = new InternetAddress(replyToStrings[i]);
    	  }
